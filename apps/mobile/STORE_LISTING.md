...
| **Purchases — Purchase History**      | Yes                    | Yes                 | No                 | RevenueCat: Pro subscription/entitlement status **and consumable AI-credit-pack purchases**. Uses: **App Functionality** (entitlement checks, granting credits, support) + **Analytics** (RevenueCat conversion / churn / revenue dashboards).                       |
| **Usage Data — Product Interaction**  | Yes                    | Yes                 | No                 | Two things: per-user usage history (items used/tossed) powering Kitchen Score / streaks / organizer tallies; and **first-party** funnel events (app opens, onboarding steps — random device id, + account id once signed in, 180-day auto-delete, no SDK). Uses: **App Functionality** + **Analytics** + **Product Personalization** (usage history feeds Shopkeeper restock hints & recipe ranking).  |
