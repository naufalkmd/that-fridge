...
class AgentToolboxTest extends TestCase
{
    ...
    public function test_mark_recipe_made_increments_the_count(): void
    {
        ...
        $this->assertStringContainsString("didn't touch your inventory", $out['content']);
    }

    private function cookieRecipe(): Recipe
    {
        return Recipe::create([
            'user_id' => $this->user->id, 'name' => 'Cookies', 'minutes' => 20,
            'ingredients' => [
                ['name' => '500g flour', 'icon' => 'leftovers'],
                ['name' => '2 eggs', 'icon' => 'eggs'],
                ['name' => '100g butter', 'icon' => 'leftovers'],
                ['name' => 'pinch of salt', 'icon' => 'leftovers'],
            ],
            'steps' => ['Mix', 'Bake'], 'made_count' => 0,
        ]);
    }

    public function test_set_recipe_consumption_plan_rejects_an_item_id_not_in_the_users_fridge(): void
    {
        $recipe = $this->cookieRecipe();

        $out = $this->toolbox->run('set_recipe_consumption_plan', [
            'recipe_id' => $recipe->id,
            'plan' => [['ingredient' => '500g flour', 'action' => 'decrement', 'item_id' => 999999, 'amount' => 1]],
        ], $this->user, $this->fridge->id);

        $this->assertFalse($out['mutated']);
        $this->assertStringContainsString('must be a real item id', $out['content']);
        $this->assertDatabaseCount('recipe_consumption_plans', 0);
    }

    public function test_set_recipe_consumption_plan_saves_a_valid_plan(): void
    {
        $recipe = $this->cookieRecipe();
        $flour = $this->item(['name' => 'Flour', 'quantity' => 2]);

        $out = $this->toolbox->run('set_recipe_consumption_plan', [
            'recipe_id' => $recipe->id,
            'plan' => [
                ['ingredient' => '500g flour', 'action' => 'decrement', 'item_id' => $flour->id, 'amount' => 1],
                ['ingredient' => 'pinch of salt', 'action' => 'skip'],
            ],
        ], $this->user, $this->fridge->id);

        $this->assertTrue($out['mutated']);
        $this->assertDatabaseHas('recipe_consumption_plans', ['user_id' => $this->user->id, 'recipe_id' => $recipe->id]);
    }

    public function test_mark_recipe_made_applies_a_saved_plan_deterministically(): void
    {
        $recipe = $this->cookieRecipe();
        $flour = $this->item(['name' => 'Flour', 'quantity' => 3]);
        $eggs = $this->item(['name' => 'Eggs', 'quantity' => 2]);
        $butter = $this->item(['name' => 'Butter', 'quantity' => 1, 'opened' => false]);

        $this->toolbox->run('set_recipe_consumption_plan', [
            'recipe_id' => $recipe->id,
            'plan' => [
                ['ingredient' => '500g flour', 'action' => 'decrement', 'item_id' => $flour->id, 'amount' => 1],
                ['ingredient' => '2 eggs', 'action' => 'decrement', 'item_id' => $eggs->id, 'amount' => 2],
                ['ingredient' => '100g butter', 'action' => 'mark_opened', 'item_id' => $butter->id],
                ['ingredient' => 'pinch of salt', 'action' => 'skip'],
            ],
        ], $this->user, $this->fridge->id);
        ...
        $this->assertTrue($out['mutated']);
        $this->assertStringContainsString('applied its consumption plan', $out['content']);
        // Decremented below its quantity - still present, count reduced.
        $this->assertSame(2, $flour->fresh()->quantity);
        // Decrement amount matched the full quantity - used up entirely, not left at 0.
        $this->assertNull(Item::find($eggs->id));
        $this->assertTrue((bool) $butter->fresh()->opened);
    }

    public function test_mark_recipe_made_plan_skips_an_item_thats_since_been_removed(): void
    {
        $recipe = $this->cookieRecipe();
        $flour = $this->item(['name' => 'Flour', 'quantity' => 1]);
