...
class IconControllerTest extends TestCase
{
    ...
    public function test_icon_generation_is_rejected_when_out_of_credits(): void
    {
        $user = User::factory()->create(['ai_credits' => 2]); // ICON costs 3

        $response = $this->actingAs($user)->postJson('/api/icons/generate', ['prompt' => 'a ripe tomato']);

        $response->assertStatus(402)->assertJson(['error' => 'insufficient_credits']);
        $this->assertDatabaseCount('generated_icons', 0);
    }

    public function test_icon_generation_spends_credits(): void
    {
        $user = User::factory()->create(['ai_credits' => 10]);
        $this->fakeSuccessfulGeneration();

        $this->actingAs($user)->postJson('/api/icons/generate', ['prompt' => 'a ripe tomato'])->assertStatus(200);
        $this->assertSame(7, $user->fresh()->ai_credits); // ICON = 3
    }

    public function test_icon_generation_refunds_the_credits_when_it_fails(): void
    {
        $user = User::factory()->create(['ai_credits' => 10]);
        $this->mock(FalClient::class, fn ($m) => $m->shouldReceive('generate')->andReturn(['ok' => false, 'reason' => 'x']));

        $this->actingAs($user)->postJson('/api/icons/generate', ['prompt' => 'x'])->assertStatus(502);
        $this->assertSame(10, $user->fresh()->ai_credits);
    }
