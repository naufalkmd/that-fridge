...
class AgentService
{
    ...
    private function getSystemPrompt($agent, $inventory = null, $usageHistory = null, $compact = false, $memory = null, $streakContext = null, $hasTools = false)
    {
        ...
        $toolsInstruction = $hasTools
            ? ' You have tools to read and act on the user\'s actual kitchen. Reads: list_items, list_notes, list_shopping, list_recipes, list_fridges, get_recipe, get_kitchen_score, list_facts. Writes: add_item, bulk_add_items (use this for several items at once), update_item, move_item, mark_item_used, add_to_shopping, check_off_shopping, remove_from_shopping, add_note, update_note, remove_note, remember_fact, forget_fact, save_recipe, mark_recipe_made, import_recipe_from_link. Deletes (confirm-first): remove_item, clear_expired_items, delete_recipe. Call list_items rather than trusting the short inventory summary above when the user asks about specific items, quantities, or what\'s expiring. When the user asks you to add, change, move, use up, buy, save or remove something, do it with the matching tool - don\'t just describe it. Use mark_item_used when they finished an item, remove_item only for something wasted or added by mistake. Only call remember_fact for lasting things (allergies, diets, habits), never one-off requests. For remove_item, clear_expired_items and delete_recipe you MUST first call the tool with confirm:false, tell the user exactly what will be deleted, and wait for them to agree in a later message before calling again with confirm:true. Never pass confirm:true on your own initiative.'
            : '';
