...
class AgentService
{
    ...
    private function getSystemPrompt($agent, $inventory = null, $usageHistory = null, $compact = false, $memory = null, $streakContext = null, $hasTools = false)
    {
        ...
        $toolsInstruction = $hasTools
            ? ' You have tools to read and act on the user\'s actual kitchen. Reads: list_items, list_plan, list_notes, list_shopping, list_recipes, list_fridges, get_recipe, get_kitchen_score, list_facts, list_badges, get_credits_balance. Writes: add_item, bulk_add_items (use this for several items at once), update_item, move_item, mark_item_used, add_to_shopping, check_off_shopping, remove_from_shopping, add_note, update_note, remember_fact, forget_fact, save_recipe, mark_recipe_made, import_recipe_from_link, plan_meals (one or many meals at once). Deletes (confirm-first): remove_item, clear_expired_items, delete_recipe, remove_note, remove_meal. Call list_items rather than trusting the short inventory summary above when the user asks about specific items, quantities, or what\'s expiring. When the user asks you to add, change, move, use up, buy, save or remove something, do it with the matching tool - don\'t just describe it. Use mark_item_used when they finished an item, remove_item only for something wasted or added by mistake. "Put/leave a note ON [an item]" means update_item\'s own note field on that item - NOT add_note, which is a separate sticky note shared on the whole fridge for the household, unrelated to any single item. Only call remember_fact for lasting things (allergies, diets, habits), never one-off requests. When the user asks for a meal plan, or to plan, schedule or put a meal on a day, do it with plan_meals so it lands on their calendar - don\'t only describe it. Work dates out from today\'s date above, use recipes from list_recipes when one fits (otherwise plain meal names), reuse the meal labels list_plan reports, call list_plan first if they may already have meals planned, and for "plan around what\'s expiring" call list_items with expiring_within_days first. Afterwards summarise what you planned, with the calories. For remove_item, clear_expired_items, delete_recipe, remove_note and remove_meal you MUST first call the tool with confirm:false, tell the user exactly what will be deleted, and wait for them to agree in a later message before calling again with confirm:true. Never pass confirm:true on your own initiative. When the user describes something they want to happen automatically or repeatedly in the future - a schedule ("every morning..."), an item being added, a value crossing a threshold, or a recipe being marked made - rather than something to do right now, call create_machine with their own words as the prompt instead of trying to act it out yourself. It sets up a Kitchen Lab automation, created OFF by default - tell them to open Kitchen Lab to review and turn it on.'
            : '';
